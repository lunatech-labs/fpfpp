...
object Swipeable {
  ...
  class Backend($ : BackendScope[Props, State]) extends OnUnmount {
    ...
    val swipeLeft = $.modState(
      _.copy(direction = Some(Direction.Left)),
      $.props.map(_.swipeLeft.delay(300.millis)).flatten.void)
    val swipeRight = $.modState(
      _.copy(direction = Some(Direction.Right)),
      $.props.map(_.swipeRight.delay(300.millis)).flatten.void)
    ...
    def render(props: Props, state: State, child: PropsChildren) = {
      ...
      val translate = state.direction match {
        case Some(Direction.Left) =>
          translateToString(-dom.window.innerWidth, state.posY)
      ...
      <.div(
        ...
        Style.swipeable,
        Style.moveBack.when(
          (atCenter && state.dragged) || state.direction.nonEmpty),
  ...
  def resetState: ComponentWillReceivePropsFn[Props, State, Backend] =
    _.setState(State())
  ...
  val component = ScalaComponent
    ...
    .configure(EventListener[KeyboardEvent]
      .install("keydown", _.backend.onKeyDown, _ => dom.window))
    .build
